...
local SettingsButton = {
    ...
    LeftBadge = { X = 5, Y = 4, Width = 25, Height = 21 },
    RightBadge = { X = 400, Y = 4, Width = 25, Height = 21 },
    RightText = { X = 420, Y = 3, Scale = 0.25 },
    SelectedSprite = { Dictionary = "commonmenu", Texture = "gradient_nav", Y = 0, Width = 431, Height = 28 },
}

---@type table
local SettingsSlider = {
    Background = { X = 250, Y = 10.0, Width = 150, Height = 9 },
    Slider = { X = 250, Y = 10.0, Width = 75, Height = 9 },
    Divider = { X = 323.5, Y = 9, Width = 2.5, Height = 10 },
    LeftArrow = { Dictionary = "commonmenutu", Texture = "arrowleft", X = 230, Y = 5, Width = 20, Height = 20 },
    RightArrow = { Dictionary = "commonmenutu", Texture = "arrowright", X = 400, Y = 5, Width = 20, Height = 20 },
}
...
function _Items:SliderProgress(Label, ProgressStart, ProgressMax, Description, Style, Enabled, Actions)
    ...
    if CurrentMenu ~= nil then
        if CurrentMenu then
            ...
            if CurrentMenu.Pagination.Minimum <= Option and CurrentMenu.Pagination.Maximum >= Option then
                ...
                local BackColor = Style.ProgressBackgroundColor or { R = 255, G = 255, B = 255, A = 255 }
                --if (type(Style.ProgressBackgroundColor) == "table") then
                RenderRectangle(CurrentMenu.X + SettingsSlider.Background.X + CurrentMenu.WidthOffset - RightOffset, CurrentMenu.Y + SettingsSlider.Background.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, SettingsSlider.Background.Width, SettingsSlider.Background.Height, BackColor.R, BackColor.G, BackColor.B, BackColor.A)
                --else
                    --error("Style ProgressBackgroundColor is not a table or undefined")
                --end
                local progColor = Style.ProgressColor or { R = 0, G = 0, B = 0, A = 255 }
                RenderRectangle(CurrentMenu.X + SettingsSlider.Slider.X + CurrentMenu.WidthOffset - RightOffset, CurrentMenu.Y + SettingsSlider.Slider.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, (((SettingsSlider.Slider.Width) / (#Items - 1)) * (ProgressStart - 1)), SettingsSlider.Slider.Height, progColor.R, progColor.G, progColor.B, progColor.A)
